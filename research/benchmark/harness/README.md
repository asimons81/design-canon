...
The harness deliberately does not contain provider credentials, paid-model calls, or a preferred agent framework. Tomorrow's execution adapter should consume one initialized run, operate within the pinned limits, store its transcript and source tree, capture declared screenshots and reports, and finalize the existing manifest.
