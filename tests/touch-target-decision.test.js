...
test('decision PR leaves 17 production rules and no F020',async()=>{const{spec,catalog}=await load();assert.equal(catalog.rules.length,17);assert.equal(catalog.rules.some(r=>r.id===spec.ruleId),false);});
